...
library(tmaptools)
library(tmap)
library(maptools)## Used for some map tools a the base coastline  --> may not still be in use
library(raster)  ## use raster to handle geoTiff  --> change to Terra packahe
require(rgeos)   ## Required for geographic manipulations, such as buffering
require(rgdal)   ## Required for gdal bindings that enable reading of GIS file formats
...
UserData <- read.csv(file = file.path(baseDir, 'UserData.csv'), 
...
## Changes 2021-01-04 due to change to the usicecenter.gov address
## old: DestFile.zip<-file.path(LocalTemp, format(Yesterday, 'nic_miz%Y%jnc_pl_a.zip'))
DestFile.zip<-file.path(LocalTemp, format(Today, 'nic_miz%Y%jnc_pl_a.zip'))
## old: DestFile.loc <- file.path(GISPantryLocation, format(Yesterday, 'NIC_MIZ/%Y'))
DestFile.loc <- file.path(GISPantryLocation, format(Today, 'NIC_MIZ/%Y'))
## old: DestFile <- file.path(DestFile.loc, format(Yesterday, 'nic_miz%Y%jnc_pl_a.shp'))
...
if(!file.exists(DestFile) | file.info(DestFile)$size < 1000){  ## if the NIC MIZ is missing or unusually small, download it again
	#base = 'http://www.natice.noaa.gov/pub/daily/arctic/'
	base = 'https://www.natice.noaa.gov/pub/daily/arctic/'
	cat('\n-----------> Attempting to download', 
	    format(Yesterday, '%Y %B %d'), 
	    fill=T)
	#cat(URL <- paste(base, tolower(format(Yesterday,'%Y//miz_shape/nic_miz%Y%jnc_pl_a.zip')) , sep =''), fill =T)
...
if(file.exists(DestFile) & file.info(DestFile)$size > 1000){  	## test for it's existence again
	...
	st_coordinates(geo180.sp) <- ~x+y # or st_as_sf(x, coords = c("x","y"))
